...
def euler_integration(motion, destination_frame, dt=1.0, return_all_frames=False):
    """
    ...
    :param dt: The time step to scale the motion values (acts as a time step like in RK).
    ...
    for frame_id in range(1, destination_frame + 1):
        destination_coords = destination_coords + dt * motion[0][:, 
            torch.round(destination_coords[1]).long(), 
        ...
        # DEBUG
        destination_coords_ = destination_coords_ + dt * motion[0][:, 
            torch.round(destination_coords[1]).long(), 
            torch.round(destination_coords[0]).long()]
...
class EulerIntegration(nn.Module):
    def __init__(self, dt=1.0):
        """
        Initialize the EulerIntegration module.

        :param dt: Time step to scale the motion values.
        """
        super().__init__()
        self.dt = dt
    ...
    def forward(self, motion, destination_frame, return_all_frames=False, show_visible_pixels=False):
        ...
        for b in range(motion.shape[0]):
            displacements[b:b + 1], visible_pixels[b:b + 1] = euler_integration(
