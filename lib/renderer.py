...
from lib.model.motion.runge_kutta_integration import RungeKuttaIntegration
from kornia.morphology import erosion


class ImgRenderer():
    def __init__(self, args, config, model, scene_flow_estimator, inpainter, device, trajectory_type, fps):
        self.args = args
        self.config = config
        self.model = model
        ...
        self.runge_kutta_integration = RungeKuttaIntegration()
        self.device = device
    ...
    def process_data(self, data):
        ...
        if torch.sum(self.mask) == 0:
            raise ValueError("Mask is entirely zero. Debug the mask creation process.")
        #else:
            #raise ValueError("Mask key is missing in the data dictionary.")
    ...
    def render_pcd(self, pts, rgbs, feats, mask, side_ids, R=None, t=None, time=0, t_step=None, path_type=None):
        ...
        # Calculate and print aggregates for debugging
        # print(f"Step {t_step}:")
        # print(f"  pts - sum: {pts.sum().item()}, mean: {pts.mean().item()}")
        # print(f"  rgbs - sum: {rgbs.sum().item()}, mean: {rgbs.mean().item()}")
        # print(f"  feats - sum: {feats.sum().item()}, mean: {feats.mean().item()}")
        # print(f"  mask - sum: {mask.sum().item()}, mean: {mask.mean().item()}")
        # print(f"  side_ids - unique values: {torch.unique(side_ids)}")
        
        rgb_feat = torch.cat([rgbs, feats], dim=-1)

        # Debugging features and points
        #print("Features passed to renderer from def render_pcd (rgb_feat):", rgb_feat.shape)
        #print("Points passed to renderer from def render_pcd (pts):", pts.shape)
        ...
        #Debugging
        #print("Original points (pts):", pts[:10])
        
        pts_ = (R.mm(pts.T) + t.unsqueeze(-1)).T
        #Debugging
        #print("Transformed points (pts_):", pts_[:10])
    ...
    def render_pcd_for_novel_view(self, pts, rgbs, feats, mask, R=None, t=None):
        ...
        #Debugging
        #print("Original points from render_pcd_for_novel_view (pts):", pts[:10])
        
        pts_ = (R.mm(pts.T) + t.unsqueeze(-1)).T
        #Debugging
        #print("Transformed points from render_pcd_for_novel_view (pts_):", pts_[:10])
    ...
    def render_rgbda_layers_from_one_view(self):
        ...
        # Debug prints for features and points
        print("Features passed to renderer from render_rgbda_layers_from_one_view (all_feats):", all_feats.shape)
        print("Points passed to renderer from render_rgbda_layers_from_one_view(all_pts):", all_pts.shape)
    ...
    def compute_flow_and_inpaint(self):
        with torch.no_grad():
            ...
            mask = self.mask[None]
            print("Mask values in cfi:", torch.sum(mask))
            hint = self.hints[None]
            outputs_motion = self.scene_flow_estimator.forward_flow(image, mask, hint)
            print("PredMotion values in cfi:", outputs_motion["PredMotion"])
            flow = outputs_motion["PredMotion"] * mask
            print("Flow values post masking in cfi:", torch.sum(flow))
            # Validate motion tensor
            assert not torch.isnan(outputs_motion["PredMotion"]).any(), "Motion tensor contains NaN values!"
            assert not torch.isinf(outputs_motion["PredMotion"]).any(), "Motion tensor contains Inf values!"

            # Validate mask tensor
            assert not torch.isnan(mask).any(), "Mask tensor contains NaN values!"
            assert not torch.isinf(mask).any(), "Mask tensor contains Inf values!"

            if torch.sum(flow) == 0:
                raise ValueError("Flow is entirely zero after initialization.")
            
            for _ in range(7):
                flow_ = box_blur(flow, (15, 15), border_type='constant')
            flow = flow_ * mask
            
            print("Flow values after box blur:", torch.sum(flow))

            flow_scale = [self.w / flow.shape[3], self.h / flow.shape[2]]
            flow = flow * torch.FloatTensor(flow_scale).to(flow.device).view(1, 2, 1, 1)
            print("Flow scale factors:", flow_scale)
            print("Flow values after scaling:", torch.sum(flow))
            # Validate flow tensor before interpolation
            assert not torch.isnan(flow).any(), "Flow tensor contains NaN values!"
            assert not torch.isinf(flow).any(), "Flow tensor contains Inf values!"

            flow = F.interpolate(flow, (self.h, self.w), mode='bilinear', align_corners=False)
            print("Flow values after Interpolation:", torch.sum(flow))
        ...
        print("Final flow values from cfi:", torch.sum(flow))
        print("Mask values at final stage from cfi:", torch.sum(self.mask))
    ...
    def only_compute_flow(self):
        with torch.no_grad():
            image = self.motion_rgbs
            mask = self.mask[None]
            print("Mask values in cfi:", mask)
            hint = self.hints[None]
            outputs_motion = self.scene_flow_estimator.forward_flow(image, mask, hint)
            print("PredMotion values in cfi:", outputs_motion["PredMotion"])
            flow = outputs_motion["PredMotion"] * mask
            print("Flow values at step X in cfi:", torch.sum(flow))
            if torch.sum(flow) == 0:
                raise ValueError("Flow is entirely zero after initialization.")
        ...
        print("Final flow values before returning from ocf:", torch.sum(flow))
        print("Mask values at final stage from ocf:", torch.sum(self.mask))
    ...
    def render_rgbda_layers_with_scene_flow(self, return_pts=False):
        ...
        with torch.no_grad():
            ...
            flow_f = self.runge_kutta_integration(motion=flow,destination_frame=self.middle_index.long() - self.start_index.long(),
                                                  trajectory_type=self.trajectory_type,# Pass trajectory type
                                                  fps=self.fps  # Pass fps for computing dt
                                                  )
            
            flow_b = self.runge_kutta_integration(motion=-flow,destination_frame=self.end_index.long() + 1 - self.middle_index.long(),
                                                  trajectory_type=self.trajectory_type,  # Pass trajectory type
                                                  fps=self.fps  # Pass fps for computing dt
                                                  )
            flow_f = flow_f.permute(0, 2, 3, 1)
            flow_b = flow_b.permute(0, 2, 3, 1)
            print("Forward flow:",flow_f)
            print("Backward flow:",flow_b)
        ...
        # Debug prints for features and points
        print("Features Flowed passed to renderer (all_feats):", all_feats_flowed.shape)
        print("Optical Flow passed to renderer (all_pts):", all_optical_flow.shape)
